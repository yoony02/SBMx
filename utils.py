...
def data_masks(all_usr_pois, item_tail, max_len=200):
    us_lens = [len(upois) for upois in all_usr_pois]
    len_max = max(us_lens)
    us_pois, us_msks = [], []
    for upois in all_usr_pois:
        new_upois = upois[-max_len:]
        le = len(new_upois)
        us_pois.append(new_upois + item_tail * (max_len - le))
        us_msks.append([1] * le + [0] * (max_len - le))
    # us_pois = [ for upois, le in zip(all_usr_pois, us_lens)]
    # us_msks = []
    # for le in us_lens:
    #     if le >= max_len:
    #         us_msks.append([1] * max_len)
    #     else:
    #         us_msks.append([1] * len_max + [0] * (len_max - le))
