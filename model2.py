...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        self.n_node = n_node
        self.norm = opt.norm
        self.lam = opt.mixup_lam
        self.mixup_pct = opt.mixup_pct
        self.ta = opt.TA
    ...
    def focal_loss(self, inputs, targets, gamma=2, alpha=None, reduce=True):
        CE = F.cross_entropy(inputs, targets, reduction='none')
        pt = torch.exp(-CE)  # pt
        loss = (1 - pt) ** gamma * CE  # -(1-pt)^rlog(pt)
        

        if alpha is not None:
            alpha = trans_to_cuda(torch.tensor(alpha, dtype=torch.float))
            # in case that a minority class is not selected when mini-batch sampling
            if len(alpha) != len(torch.unique(targets)):
                temp = trans_to_cuda(torch.zeros(len(alpha)))
    ...
    def norm_mixup(self, feats, targets):
        mixup_samples = int(feats.shape[0] * self.mixup_pct)
        norms = torch.norm(feats, dim=1)
        snorm_idxs = (-norms).topk(mixup_samples)[1]
        lnorm_idxs = norms.topk(mixup_samples)[1]
        mix_sess = feats[snorm_idxs] * self.lam + feats[lnorm_idxs] * (1 - self.lam)
        y_as, y_bs = targets[snorm_idxs], targets[lnorm_idxs]
        
        return mix_sess, y_as, y_bs
    
    def mixup_loss_function(self, pred, y_as, y_bs):
        # return self.loss_function(pred, y_as) * self.lam + self.loss_function(pred, y_bs) * (1 - self.lam)
        return self.focal_loss(pred, y_as) * self.lam + self.focal_loss(pred, y_bs) * (1 - self.lam)
...
def forward(model, i, data, ht_dict, train=True):
    ...
    if train:
        ...
        mixup_feats, y_as, y_bs = model.norm_mixup(feats, targets)
        scores = model.compute_score(feats)
        mixup_scores = model.compute_score(mixup_feats)

        return scores, targets, mixup_scores, y_as, y_bs, tail_idxs
    else:
        hidden = model(items, A)
        feats = model.session_encoding(hidden, alias_inputs, mask)
        scores = model.compute_score(feats)
        
        return scores, targets, tail_idxs    
...
def train_test(model, train_data, test_data, n_node, ht_dict, pop_dict, Ks=[10, 20]):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        scores, targets, mixup_scores, y_as, y_bs, _ = forward(model, i, train_data, ht_dict)
        loss = model.focal_loss(scores, targets)
        loss += model.mixup_loss_function(mixup_scores, y_as, y_bs)
        loss.backward()
    ...
    for i in slices:
        scores, targets, tail_idxs = forward(model, i, test_data, ht_dict, train=False)
